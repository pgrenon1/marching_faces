...
As of dsj 0_8_6 the dll is also available as a 64bit build. 
...
implementing dsj. Also please check the website for updates.
...
b_0_88
More interface implementations
New DSMovie constructor taking any number of filters to build graph from
Fixes variable scope bug in JavaSourceFilter
Fixes DSStreamBufferGraph on Windows 8+
Fixes problems in TSNetworkSource with transportstreams over http and audio only streams
HTTPStreaming & DASHSources more functional (though still far from complete)
Adds basic HEVC support to MKV & Transportstream sources (HEVC handling in DirectShow is still pretty much unclear in terms of FCCs, mediatype formatting etc.)

b_0_87
Some additional interface implementations

Windows 8 notes:
Some decoders that were globally useable in Windows 7 are now restricted and can no longer be used freely. This
affects (a.o.) DVD playback. See this msdn blog post for details: 
http://blogs.msdn.com/b/b8/archive/2012/05/04/q-amp-a-dvd-playback-and-windows-media-center-in-windows-8.aspx 

b_0_8_64
media subtype GUID field added to DSMediaType
some bugfixes, missing implementation catch-ups in various RTPHandlers
some additional api in DSDvd 
contains a first version of a DASH (Dynamic Adaptive Streaming over HTTP) source

b_0_8_63
added RendererControls object for EVR 
wav / mp3 sink added in FileSink.forType(..)
H264 RTPHandler no longer depends on SPS/PPS in sdp
propper AVC / H264 cropping support added for all source classes
AVCParser correctly reads high profile sequence scaling matrices 
PMT lookup timeout in M2TS RTPHandler extended
workarounds for some Taiwanese interpretations of sdp parameter spelling
full device paths of audio filters (which are not directly available with DirectShow) now retrieved from Wave api to enable lookup of corresponding devices
Support for DMO Audio (and some Video) effects
OverlayFilter now supports static overlays without constant refreshs as well as partial updates
fixed problem with onSDES metadata events used by latest Wowza builds
partly breaks backward compatibility with jdk 1.4 and 1.5. Most things should still be runnable
