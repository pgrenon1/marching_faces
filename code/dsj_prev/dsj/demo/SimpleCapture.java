	...
	public void createGraph() {
		...
		if (graph.getActiveVideoDevice() != null) {
			...
			try {
				...
				for (int i = CaptureDeviceControls.BRIGHTNESS; i < CaptureDeviceControls.LT_FINDFACE; i++) try { jf.add(cdc.getController(i, 0, true)); }catch (Exception ex){}
		...
		if (graph.getActiveAudioDevice() != null) {
			...
			try {
				...
				for (int i = CaptureDeviceControls.MASTER_VOL; i < CaptureDeviceControls.TREBLE; i++) try { jf.add(acdc.getController(i, 0, true)); }catch (Exception ex){}
